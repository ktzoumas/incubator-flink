...
/**
 * {@link FlinkUnorderedKVInput} provides unordered key value input by
...
public class FlinkUnorderedKVInput extends AbstractLogicalInput {

	private static final Log LOG = LogFactory.getLog(FlinkUnorderedKVInput.class);
	...
	private FlinkUnorderedKVReader kvReader;
	...
	public FlinkUnorderedKVInput(InputContext inputContext, int numPhysicalInputs) {
	...
	public synchronized void start() throws IOException {
		if (!isStarted.get()) {
			...
			try {
				typeSerializer = (TypeSerializer<?>) EncodingUtils.decodeObjectFromString(conf.get("io.flink.typeserializer"), getClass().getClassLoader());
			}
			catch (Exception e) {
			...
			this.kvReader = createReader(inputRecordCounter, codec,
					ifileBufferSize, ifileReadAhead, ifileReadAheadLength, typeSerializer);
	...
	@SuppressWarnings("rawtypes")
	private FlinkUnorderedKVReader createReader(TezCounter inputRecordCounter, CompressionCodec codec,
								int ifileBufferSize, boolean ifileReadAheadEnabled, int ifileReadAheadLength,
								TypeSerializer<?> typeSerializer)
			throws IOException {
		return new FlinkUnorderedKVReader(shuffleManager, conf, codec, ifileReadAheadEnabled,
				ifileReadAheadLength, ifileBufferSize, inputRecordCounter, typeSerializer);
	}
