...
@Unstable
@Private
public class FlinkUnorderedKVReader<K, V> extends KeyValueReader {

	private static final Log LOG = LogFactory.getLog(FlinkUnorderedKVReader.class);
	...
	private final Deserializer<WritableSerializationDelegate<V>> valDeserializer;
	...
	private K key;
	private WritableSerializationDelegate<V> value;
	...
	public FlinkUnorderedKVReader(ShuffleManager shuffleManager, Configuration conf,
						CompressionCodec codec, boolean ifileReadAhead, int ifileReadAheadLength, int ifileBufferSize,
						TezCounter inputRecordCounter, TypeSerializer<V> valueSerializer)
			throws IOException
	{
		...
		this.value = new WritableSerializationDelegate<V>(valueSerializer);
		...
		this.valDeserializer = (Deserializer<WritableSerializationDelegate<V>>) serializationFactory.getDeserializer(valClass);
