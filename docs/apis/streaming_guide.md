...
The following program is a complete, working example of streaming window word count application, that incrementally counts the
...
dataStream.windowAll(TumblingTimeWindows.of(Time.of(5, TimeUnit.SECONDS))); // Last 5 seconds of data
  {% endhighlight %}
        ...
        <tr>
          ...
          <td>
            <p>Applies a general function to the window as a whole. A function that manually sums the elements
	    of a window</p>
...
windowedStream.reduce (new ReduceFunction<Tuple2<String,Integer>() {
    ...
    {% endhighlight %}
        ...
        <tr>
          ...
          <td>
            <p>Aggregates the contents of a window</p>
    {% highlight java %}
TODO
    {% endhighlight %}
          </td>
...
DataStream<Integer> all = split.select("even","odd");
        ...
        <tr>
          <td><strong>Iterate</strong><br>DataStream &rarr; IterativeStream</td>
          <td>
            <p>
...
DataStream<Long> output = iterationBody.filter(new FilterFunction<Long>(){
    ...
    public boolean filter(Integer value) throws Exception {
        ...
        <tr>
          ...
          <td>
            <p>
                Extract timestamps from records in order to work with Flink windows
                using event time semantics. See <a href="#working-with-time">working with time</a>.
                {% highlight java %}
stream.extractTimestamps (new TimeStampExtractor() {...});
                {% endhighlight %}
            </p>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <p>
                ...
                for details. See <a href="#working-with-time">working with time</a>.
                {% highlight scala %}
stream.extractTimestamps { timestampExtractor }
                {% endhighlight %}
            </p>
          </td>
        </tr>
...
- `fromCollection(Seq)` - Creates a data set from the Java Java.util.Collection. All elements
  in the collection must be of the same type.

- `fromCollection(Iterator)` - Creates a data set from an iterator. The class specifies the
  data type of the elements returned by the iterator.

- `fromElements(elements: _*)` - Creates a data set from the given sequence of objects. All objects must be
  of the same type.
...
Before running a streaming program on a large data set in a distributed cluster, it is a good
...
The default value is `TimeCharacteristic.ProcessingTime`. This will make sure that all transformations 
(unless explicitly specified in the transformation name) will use the desired notion of time. 

In order to work with event time semantics, you need to follow three steps:

- Set `env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime)`

- Use `DataStream.extractTimestamps(...)` in order to tell Flink how timestamps relate to events (e.g., which
...
stream.extractTimestamps(new TimeStampExtractor<Tuple4<Long,Integer,Double,String>>{
    @Override
    ...
    @Override
    public long emitWatermark(Tuple4<Long,Integer,Double,String> element, long currentTimestamp) {
...
stream.extractTimestamp(new TimestampExtractor[(Long, Int, Double, String)] {
  override def extractTimestamp(element: (Long, Int, Double, String), currentTimestamp: Long): Long = element._1

  override def emitWatermark(element: (Long, Int, Double, String), currentTimestamp: Long): Long = element._1 - 1000
...
{% highlight java %}
DataStream<Tuple4<Long,Integer,Double,String>> stream = //...
stream.extractTimestamps(new AscendingTimeStampExtractor<Tuple4<Long,Integer,Double,String>>{
    @Override
...
time starts from 0. Then, we have 10 windows that overlap: [0,1000], [100,1100], [200,1200], ..., [1000, 2000]. Each incoming
...
stream.window(TumblingTimeWindows.of(Time.of(1, TimeUnit.SECONDS)));
      ...
      {% highlight java %}
stream.window(TumblingProcessingTimeWindows.of(Time.of(1, TimeUnit.SECONDS)));
      {% endhighlight %}
          </td>
        </tr>
      <tr>
    ...
    {% endhighlight %}
      ...
      <tr>
          ...
          <td>
            <p>
              Incoming elements are assigned to a window of a certain size (5 seconds below) based on
              the current processing time. Windows "slide" by the provided value (1 second in the example), and hence
